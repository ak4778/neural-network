...
void NRTypeInsertData(NRTypeObject *o, float *inputs, float *outputs, int target_ds) {
    ...
    for (j = 0; j < numin; j++) {
        printf("target->inputs[%d] = %f\n",j,inputs[j]); 
        target->inputs[idx*numin+j] = inputs[j];
    }
    for (j = 0; j < numout; j++) {
        printf("target->outputs[%d] = %f\n",j,outputs[j]); 
...
void *NRTrainingThreadMain(void *arg) {
    ...
    for(int j = 0;j < nr->dataset.len; ++j) {
        for (int i = 0;i < INPUT_UNITS(nr->nn);++i)
        {
            if (i == 0) {
                if (INPUT_UNITS(nr->nn) == 1)
                    printf("input[%d][%d] (%f)", j,i, nr->dataset.inputs[j*INPUT_UNITS(nr->nn)+i]);
                else
                    printf("input[%d][%d] (%f,", j,i, nr->dataset.inputs[j*INPUT_UNITS(nr->nn)+i]);
            }
            else if (i == (INPUT_UNITS(nr->nn)-1))
                printf("%f)", nr->dataset.inputs[j*INPUT_UNITS(nr->nn)+i]);
            else
                printf("%f,", nr->dataset.inputs[j*INPUT_UNITS(nr->nn)+i]);
        }
        for (int i = 0;i < OUTPUT_UNITS(nr->nn);++i)
        {
            if (i == 0) {
                if (OUTPUT_UNITS(nr->nn) == 1)
                    printf(" -> (%f)\n", j,i, nr->dataset.outputs[j*OUTPUT_UNITS(nr->nn)+i]);
                else
                    printf(" -> (%f,", j,i, nr->dataset.outputs[j*OUTPUT_UNITS(nr->nn)+i]);
            }
            else if (i == (OUTPUT_UNITS(nr->nn)-1))
                printf("%f)\n", nr->dataset.outputs[j*OUTPUT_UNITS(nr->nn)+i]);
            else
                printf("%f,", nr->dataset.outputs[j*OUTPUT_UNITS(nr->nn)+i]);
        }
    }
    if ((nr->flags & NR_FLAG_NORMALIZE) && nr->dataset.len) {
        ...
        for (int i = 0; i < ilen; i++) {
            if (imax[i] != 1) imax[i] *= 1.2;
            printf("imax[%d] = %f\n",i,imax[i]);
        }
        for (int i = 0; i < olen; i++) {
            if (omax[i] != 1) omax[i] *= 1.2;
            printf("omax[%d] = %f\n",i,omax[i]);
        }
    ...
    int ilen = INPUT_UNITS(nr->nn);
    int olen = OUTPUT_UNITS(nr->nn);
    for(int i = 0;i < ilen*nr->dataset.len; ++i)
    {
        printf("dataset.inputs[%d] = %f, inorm = %f\n",i, nr->dataset.inputs[i], nr->inorm[i/nr->dataset.len]);
    }

    for(int i = 0;i < olen*nr->dataset.len; ++i)
    {
        printf("dataset.outputs[%d] = %f, inorm = %f\n",i, nr->dataset.outputs[i], nr->onorm[i/nr->dataset.len]);
    }
...
int NRCreate_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    while (j < argc) {
        ...
        if (RedisModule_StringToLongLong(argv[j],&units) != REDISMODULE_OK || units <= 0)
        {
    ...
    NRTypeObject *nr = createNRTypeObject(flags,layers,num_layers,dset_size,test_size);
...
int NRGetdata_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (!strcasecmp(RedisModule_StringPtrLen(argv[3],NULL),"all")) {

        /* Send inputs */
        RedisModule_ReplyWithArray(ctx,(ilen+olen+3)*target->len);

        for(int i = 0; i < target->len; i++) {

            /* Send inputs */
            RedisModule_ReplyWithSimpleString(ctx,"input");
            for(int j = 0; j < ilen; j++) {
                double input = target->inputs[ilen*i+j];
            ...
            for(int j = 0; j < olen; j++) {
                double output = target->outputs[olen*i+j];
                RedisModule_ReplyWithDouble(ctx,output);
            }
            RedisModule_ReplyWithSimpleString(ctx,"--------------");
        }
