...
void AnnFree(struct Ann *net)
{
    ...
    /* Free layer data */
    for (i = 0; i < net->num_layers; i++) AnnFreeLayer(&net->layer[i]);
...
/*
 * Input layer and Hidden layer memory map:
        u = net->layer[i].units;
        j = net->layer[i-1].units;
        layer[i]->output  ------> |X0|X1|X2|...|Xu|
        layer[i]->error   ------> |E0|E1|E2|...|Eu|
        layer[i]->weight  ------> |W0|W1|W2|...|Wu|   X[i-1][0] i-1层的第0个输出权重集合
                                  |W0|W1|W2|...|Wu|   X[i-1][1] i-1层的第1个输出权重集合
                                  |W0|W1|W2|...|Wu|   X[i-1][2] i-1层的第2个输出权重集合
                                     ...
                                  |W0|W1|W2|...|Wu|   X[i-1][j] i-1层的第j个输出权重集合

        layer[i]->gradient------> |G0|G1|G2|...|Gu|   X[i-1][0] i-1层的第0个gradient
                                  |G0|G1|G2|...|Gu|   X[i-1][1] i-1层的第1个gradient
                                  |G0|G1|G2|...|Gu|   X[i-1][2] i-1层的第2个gradient
                                     ...
                                  |G0|G1|G2|...|Gu|   X[i-1][j] i-1层的第j个gradient

        layer[i]->Sgradient-----> |S0|S1|S2|...|Su|   X[i-1][0] i-1层的第0个sgradient
                                  |S0|S1|S2|...|Su|   X[i-1][1] i-1层的第1个sgradient
                                  |S0|S1|S2|...|Su|   X[i-1][2] i-1层的第2个sgradient
                                     ...
                                  |S0|S1|S2|...|Su|   X[i-1][j] i-1层的第j个sgradient

        layer[i]->pgradient-----> |P0|P1|P2|...|Pu|   X[i-1][0] i-1层的第0个pgradient
                                  |P0|P1|P2|...|Pu|   X[i-1][1] i-1层的第1个pgradient
                                  |P0|P1|P2|...|Pu|   X[i-1][2] i-1层的第2个pgradient
                                     ...
                                  |P0|P1|P2|...|Pu|   X[i-1][j] i-1层的第j个pgradient

        layer[i]->delta    -----> |D0|D1|D2|...|Du|   X[i-1][0] i-1层的第0个delta
                                  |D0|D1|D2|...|Du|   X[i-1][1] i-1层的第1个delta
                                  |D0|D1|D2|...|Du|   X[i-1][2] i-1层的第2个delta
                                     ...
                                  |D0|D1|D2|...|Du|   X[i-1][j] i-1层的第j个delta


   layer[0] is the output layer, its' map is simple:
        layer[0]->output  ------> |O0|O1|O2|...|Ou|
        layer[0]->error   ------> |E0|E1|E2|...|Eu|
*/
int AnnInitLayer(struct Ann *net, int i, int units, int bias) {
    ...
    if (i) { /* not for output layer */
        j = net->layer[i-1].units;
        net->layer[i].weight    = malloc(sizeof(float)*units*j);
        net->layer[i].gradient  = malloc(sizeof(float)*units*j);
        net->layer[i].pgradient = malloc(sizeof(float)*units*j);
        net->layer[i].delta     = malloc(sizeof(float)*units*j);
        net->layer[i].sgradient = malloc(sizeof(float)*units*j);
        int size =  units*net->layer[i-1].units;
        //printf("alloc[%d] = %d\n",i, size);
    }
    ...
    if (i) {
        memset(net->layer[i].weight,    0, sizeof(float)*units*net->layer[i-1].units);
        memset(net->layer[i].gradient,  0, sizeof(float)*units*net->layer[i-1].units);
        memset(net->layer[i].pgradient, 0, sizeof(float)*units*net->layer[i-1].units);
        memset(net->layer[i].delta,     0, sizeof(float)*units*net->layer[i-1].units);
        memset(net->layer[i].sgradient, 0, sizeof(float)*units*net->layer[i-1].units);
    }
...
void AnnSimulate(struct Ann *net) {
    ...
    for (i = net->num_layers-1; i > 0; i--) {
        ...
        for (j = 0; j < nextunits; j++) {
            ...
            k = 0;
/*
 *      the memory map is just like following:
 *      layer i has u output as the input for layer i-1 which has j output
        u = net->layer[i].units;
        j = net->layer[i-1].units;
        layer[i]->error   ------> |E0|E1|E2|...|Eu|
        layer[i]->output  ------> |X0|X1|X2|...|Xu|
        layer[i]->weight  ------> |W0|W1|W2|...|Wu|   O[i-1][0] i-1层的第0个输出权重集合
                                  |W0|W1|W2|...|Wu|   O[i-1][1] i-1层的第1个输出权重集合
                                  |W0|W1|W2|...|Wu|   O[i-1][2] i-1层的第2个输出权重集合
                                     ...
                                  |W0|W1|W2|...|Wu|   O[i-1][j] i-1层的第j个输出权重集合
                        
               A = layer[i-1][j] = W0*X0 + W1*X1 + W2*X2 + ... + Wu*Xu
...
//    printf("factor = %f\n",factor);
    int j;
    for (j = 0; j < units; j++) {
//        printf(" %d o = %f, d = %f\n",j,net->layer[0].output[j], desired[j]);
...
void AnnCalculateGradients(struct Ann *net, float *desired) {
    ...
    /* layer 0 is the output layer, it only contains output and error */
    for (j = 0; j < layers; j++) {
        ...
        for (i = 0; i < units; i++) {
            ...
            float *gprev = prev_layer->gradient + i*prevunits;
            float *wprev = prev_layer->weight + i*prevunits;
            float *oprev = prev_layer->output;
            float *eprev = prev_layer->error;

            /* 1. Calculate the gradient */
            k = 0;

            for (; k < prevunits; k++) *gprev++ = error_signal*(*oprev++);

            /* 2. And back-propagate the error to the previous layer */
            k = 0;

            for (; k < prevunits; k++) {
                *eprev += error_signal * (*wprev);
                eprev++;
                wprev++;
            }
        }
...
void AnnAdjustWeightsResilientBP(struct Ann *net) {
    ...
    for (j = 1; j < layers; j++) {
        ...
        for (i = 0; i < weights; i++) {
            float t = net->layer[j].pgradient[i] * net->layer[j].sgradient[i];
...
static int AnnTestClassificationsError(struct Ann *net, float *desired) {
    int i, outputs = OUTPUT_UNITS(net);
    int classid, outid;
    float max = 0;
...
void AnnTestError(struct Ann *net, float *input, float *desired, int setlen, float *avgerr, float *classerr) {
    ...
    for (j = 0; j < setlen; j++) {
        error += AnnSimulateError(net, input, desired);
        if (classerr)
            class_errors += AnnTestClassificationsError(net, desired);
        input += inputs;
        desired += outputs;
    }
